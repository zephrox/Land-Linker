...
function db_admin_users(mysqli $conn, string $q = ''): array {
  if ($q !== '') {
    ...
    $sql = "SELECT u.*, r.name AS role_name
            ...
            WHERE u.email LIKE '{$like}' OR u.first_name LIKE '{$like}' OR u.surname LIKE '{$like}'
            ORDER BY u.id DESC";
...
function db_admin_update_user_password(mysqli $conn, int $id, string $password_plain): void {
  ...
  mysqli_query($conn, "UPDATE users SET password_hash='{$hash}' WHERE id={$id}");
}
...
function db_manager_users_all(mysqli $conn, string $q = ''): array {
  $like = '%' . mysqli_real_escape_string($conn, $q) . '%';
  $sql = "SELECT u.id, u.first_name, u.surname, u.email, u.status,
                 r.name AS role_name, u.role_id
          FROM users u
          JOIN roles r ON r.id = u.role_id
          WHERE u.first_name LIKE '{$like}' OR u.surname LIKE '{$like}' OR u.email LIKE '{$like}'
          ORDER BY u.id DESC";
  $res = mysqli_query($conn, $sql);
  return $res ? mysqli_fetch_all($res, MYSQLI_ASSOC) : [];
}
...
function db_manager_create_user(mysqli $conn, array $d): int {
  ...
  $first   = mysqli_real_escape_string($conn, (string)$d['first_name']);
  $sur     = mysqli_real_escape_string($conn, (string)$d['surname']);
  $email   = mysqli_real_escape_string($conn, (string)$d['email']);
  $phone   = mysqli_real_escape_string($conn, (string)$d['phone']);
  $hash    = mysqli_real_escape_string($conn, (string)$d['password_hash']);
  $status  = mysqli_real_escape_string($conn, (string)$d['status']);
...
function db_manager_delete_user(mysqli $conn, int $user_id): bool {
  $user_id = (int)$user_id;
  return (bool)mysqli_query($conn, "DELETE FROM users WHERE id={$user_id}");
}
