...
function db_create_schedule(mysqli $conn, int $user_id, string $title, string $starts_at, string $ends_at, string $color_tag='blue'): int {
  ...
  $starts_at = mysqli_real_escape_string($conn, $starts_at);
  $ends_at = mysqli_real_escape_string($conn, $ends_at);
  $color_tag = mysqli_real_escape_string($conn, $color_tag);

  $sql = "INSERT INTO schedules (user_id,title,starts_at,ends_at,color_tag)
